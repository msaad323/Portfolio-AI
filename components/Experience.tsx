    ...
    >
      <div className="mx-auto max-w-5xl px-4">
        ...
        <div className="relative">
          ...
          <div className="absolute left-3 top-0 h-full w-[2px] bg-gradient-to-b from-sky-500 via-emerald-400 to-slate-800 md:left-1/2 md:-ml-[1px]" />
              ...
              {roles.slice(0, visibleCount).map((role, index) => (
                ...
                >
                  ...
                  <div className={`hidden md:flex flex-col ${index % 2 === 0 ? "order-3 items-start text-left" : "order-1 items-end text-right"}`}>
                    <p className="text-[10px] font-bold text-slate-100 uppercase tracking-[0.2em]">{role.period}</p>
                  ...
                  <div className="relative order-1 md:order-2 flex justify-start md:justify-center">
                    <div className="z-10 flex h-6 w-6 items-center justify-center rounded-full bg-slate-950">
                      <div className={`h-3 w-3 rounded-full border-2 border-slate-950 ${role.isCurrent ? "bg-emerald-400 shadow-[0_0_15px_rgba(52,211,153,0.6)]" : "bg-sky-400"}`} />
                      {role.isCurrent && (
                    ...
                    <div className="rounded-3xl border border-slate-800/80 bg-slate-900/40 p-6 shadow-2xl backdrop-blur-sm transition-all hover:-translate-y-1 hover:border-sky-500/30">
                      <div className="flex flex-col gap-1">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          ...
                          <span className="md:hidden text-[9px] font-bold text-sky-400/80 bg-sky-500/5 px-2 py-0.5 rounded-full border border-sky-500/20 uppercase tracking-tighter">
                            {role.period}
                          </span>
                        </div>
                        <p className="text-[11px] font-bold text-sky-400 mt-1">
                          {role.company}
                        </p>
                        <p className="md:hidden text-[10px] text-slate-500 italic mt-0.5">
                          {role.location}
                        </p>
                      </div>

                      <ul className="mt-4 space-y-2.5 text-[11px] text-slate-300 leading-relaxed">
                        {role.highlights.map((h, i) => (
                          <li key={i} className="flex gap-2.5">
                            <span className="mt-1.5 h-1 w-1 flex-shrink-0 rounded-full bg-sky-500/40" />
                            <span>{h}</span>
                          </li>
                        ))}
                      </ul>
                      ...
                      <div className="mt-6 flex flex-wrap gap-1.5">
                        {role.stack.map((tech) => (
                          <span
                            key={tech}
                            className="rounded-md border border-slate-800/80 bg-slate-800/30 px-2 py-0.5 text-[9px] font-medium text-slate-400 hover:text-sky-300 hover:border-sky-500/20 transition-colors"
                          >
                            {tech}
                          </span>
                        ))}
          ...
          {visibleCount < roles.length && (
            ...
            >
              <button
                onClick={showMore}
                className="group relative z-20 flex flex-col items-center gap-2 bg-slate-950 px-6 text-slate-500 transition-all hover:text-sky-400"
              >
